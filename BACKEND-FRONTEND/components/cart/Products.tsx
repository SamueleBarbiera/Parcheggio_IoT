...
const Products = ({ products }: any) => {
    ...
    const { data, error, isValidating } = useSWR(
        `${process.env.NEXT_URL}/api/variantetaglias?populate=*&sort=price:${order}&sort=createdAt:${order}&pagination[page]=${pageIndex}&pagination[pageSize]=4`,
        fetcher,
        {
    ...
    return (
        <main className="my-12 flex h-full items-center justify-center xl:h-screen ">
            {isValidating ? (
                <div className="mx-4  flex h-screen w-screen items-center justify-center rounded-lg bg-indigo-200 px-2 py-8 shadow-xl smd:mx-12 smd:px-4">
                    <div className="mx-auto flex flex-col items-center justify-center space-x-1 text-4xl font-semibold">
                        <RefreshIcon className="m-2 h-12 w-12 flex-shrink-0 animate-spin rounded-full bg-indigo-100 py-2 text-indigo-800 " />
                ...
                <div className="mx-4 h-full rounded-xl bg-indigo-200 px-2 py-8 shadow-xl smd:mx-12 smd:px-4">
                    {/* Mobile dialog */}
                                    ...
                                    <Menu as="div" className="relative mr-8 inline-block text-left">
                                        ...
                                        >
                                            <Menu.Items className="absolute left-0 mt-2 w-40 origin-top-left rounded-md bg-gray-50 shadow-2xl ring-1 ring-black ring-opacity-5 focus:outline-none">
                                                <div className="py-1">
                                                    <button
                                                        className={`mr-4 block rounded p-2 px-4 py-2 text-sm text-black md:p-2 ${order === 'DESC' ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                                                        disabled={order === 'DESC'}
                                                    ...
                                                    <button
                                                        className={`flex flex-auto rounded p-2 py-2 px-4 text-sm text-black md:p-2 ${order === 'ASC' ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                                                        disabled={order === 'ASC'}
                                                ...
                                                {Filtri.map((section, sectionIdx) => (
                                                    <Popover key={section.name} className="relative inline-block px-4 text-left">
                                                        <Popover.Button className="group inline-flex justify-center text-sm font-medium text-gray-700 hover:text-gray-900">
                                                            ...
                                                            {sectionIdx === 0 ? (
                                                                <span className="ml-1.5 rounded bg-indigo-900 py-0.5 px-1.5 pt-1 text-xs font-semibold tabular-nums text-gray-700">1</span>
                                                            ) : null}
                                                            <ChevronDownIcon className="-mr-1 ml-1 h-5 w-5 flex-shrink-0 text-gray-400 group-hover:text-gray-500" aria-hidden="true" />
                                                        </Popover.Button>
                        ...
                        <div className=" mx-auto flex max-w-2xl flex-col items-center px-4 sm:px-6 lg:max-w-7xl lg:px-8">
                            <div className="mx-4 grid grid-cols-1 gap-y-8 gap-x-12  md:mx-0 md:grid-cols-2 xl:grid-cols-4">
                                {data.data.map((product: any) => (
                                    <a className="group rounded-xl border bg-indigo-200 p-6 shadow-xl">
                                        <div className="h-auto  w-auto items-center justify-between p-2 group-hover:scale-105 group-hover:transform group-hover:duration-200 group-hover:ease-in-out">
                                            {isValidating ? (
                                                <div className="mx-auto h-auto w-full rounded-lg bg-indigo-200  py-4  px-4 shadow-xl">
                                                    <div className="flex flex-col items-center space-x-1 text-4xl font-semibold">
                                                        <RefreshIcon className="m-2 h-12 w-12 flex-shrink-0 animate-spin rounded-full bg-indigo-100 py-2 text-indigo-800 " />
                                                        <p className="mt-3 animate-pulse text-lg">Caricamento . . .</p>
                                                    </div>
                                                ...
                                                <Link href={`/Prodotti/${product.slug}`} key={product.slug}>
                                                    <img className="grid h-auto w-full rounded-xl border shadow-md" src={process.env.NEXT_URL + '' + product.image.data[0].url} alt={'not found'} />
                                                </Link>
                                            )}
                                            {/* {product.image.data.map((image: any) => (
                                                    <img className="flex  h-auto w-24 flex-row justify-between" src={process.env.NEXT_URL + '' + image.url} alt={'not found'} />
                                                ))} 
                                            */}
                                        </div>
                                        ...
                                        <div className="mt-4 flex items-center justify-between space-x-2">
                                            <div>
                                                <p className="text-indigo-500">Prezzo</p>
                        ...
                        <div className="flex flex-col items-end  space-x-2 px-6">
                            <div className="flex sm:flex-1 sm:items-center sm:justify-center">
                                <nav className="relative z-0 inline-flex -space-x-px  shadow-lg" aria-label="Pagination">
                                    <button
                                        className={`rounded-l-md p-2 py-2 text-black  md:p-2 ${pageIndex === 1 ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                                    ...
                                    {Array.from(Array(data.meta.pagination.pageCount + 1).keys())
                                        ...
                                        .map((numbers) => (
                                            <button
                                                className={` transiction relative hidden items-center px-4 py-2 text-sm font-semibold text-indigo-900  duration-200 ease-in-out hover:bg-indigo-900 md:inline-flex ${
                                                    pageIndex === numbers ? 'bg-indigo-900' : 'bg-indigo-900 text-indigo-100 opacity-90 contrast-50'
                                                }`}
                                    ...
                                    <button
                                        className={`rounded-r-md p-2 py-2 text-black  md:p-2 ${pageIndex === (data && data.meta.pagination.pageCount) ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                            ...
                            <div>
                                <button
                                    className={`mr-4 rounded p-2 py-2 text-black md:p-2 ${order === 'DESC' ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                                    disabled={order === 'DESC'}
                                    onClick={() => setOrder('DESC')}
                                >
                                    DESC
                                ...
                                <button
                                    className={`rounded p-2 py-2 text-black  md:p-2 ${order === 'ASC' ? 'bg-indigo-300' : 'bg-indigo-900'}`}
                                    disabled={order === 'ASC'}
                                    onClick={() => setOrder('ASC')}
                                >
                                    ASC
